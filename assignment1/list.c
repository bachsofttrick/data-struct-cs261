...
void list_free(struct list* list)
{
     ...
     * FIXED ME: 
     */
    // Check if the argument isn't a linked list
    if (list == NULL) return;
    /* A while loop with currPointer as counter is used to go through each node for deletion
     */
    struct node *currPointer = list->head;
    struct node *prevPointer = NULL;
    while (currPointer != NULL) {
        // Save the next node to counter
...
void list_insert(struct list* list, void* val)
{
    ...
    newNode->val = val;
    /* Check if there is a node already, if there is, replace the head with the new node and
     * the old head becomes the next node of the new head. Otherwise, the new node is the head.
     */
    if (list->head != NULL) {
...
void list_insert_end(struct list* list, void* val)
{
    ...
    /* Find the tail node using a temporary pointer (currPointer)
     * A while loop is used to find the tail node, i.e. the one with the NULL next pointer
     */
    struct node *currPointer = list->head;
...
void list_remove(struct list* list, void* val, int (*cmp)(void* a, void* b))
{
    ...
    struct node *prevPointer = NULL;
    while (currPointer != NULL) {
        if (cmp(val, currPointer->val) == 0) {
            if (prevPointer == NULL) {
                // Remove node from the list (the head node).
                list->head = currPointer->next;
            } else {
                // Remove node from the list (not the head).
                prevPointer->next = currPointer->next;
            }
            currPointer->next = NULL;
            free(currPointer);
            // Break loop because we only need to remove the first instance
            break;
        }
        // Save the next node to counter.
        prevPointer = currPointer;
...
void list_remove_end(struct list* list)
{
     ...
     * FIXED ME: 
     */
    // Check if the argument isn't a linked list or the list is empty
    ...
    /* A while loop with currPointer as counter is used to find the node before the tail node
     */
    while (currPointer->next->next != NULL) {
        currPointer = currPointer->next;
    }
    free(currPointer->next);
    currPointer->next = NULL;
    return;
}
...
int list_position(struct list* list, void* val, int (*cmp)(void* a, void* b))
{
    ...
    /* A while loop with currPointer as counter is used to go through each node to find the specified value
     */
    struct node *currPointer = list->head;
    int index = -1;
    while (currPointer != NULL) {
    ...
    // Return if it can't find the specified value
    return -1;
}
...
void list_reverse(struct list* list)
{
    ...
    // Check if the argument isn't a linked list
    if (list == NULL) return;
    /* A while loop with currPointer as counter is used to go through each node for reversal
     */
    struct node *currPointer = list->head;
    struct node *prevPointer = NULL;
    while (currPointer != NULL) {
        ...
        if (prevPointer == NULL) {
            /* Put the current node as the previous one.
             * Save the next node to counter
            ...
            /* Connect current node to the previous one.
             * Put the current node as the new previous one.
             * Save the next node to counter.
             */
            struct node *currPointer2 = currPointer->next;
            currPointer->next = prevPointer;
            prevPointer = currPointer;
            currPointer = currPointer2;
        }
    }
    /* Put the tail node as the new head
     * If the list is empty, the head is still NULL
     */
    list->head = prevPointer;
    return;
}
