 ...
 * this implementation.
 */
/* Some modifications have been made to be used in queue */
...
struct dynarray {
  ...
  int size;
  int capacity;
  // Start index to keep track of start of data
  int start;
};
...
/*  
 * This auxilliary function calculates the physical index from a logical index in a dynamic array
 */
int _dynarray_get_physical_idx(struct dynarray* da, int idx) {
  assert(da);
  assert(idx >= 0);

  return (da->start + idx) % da->capacity;
}

/*  
 * This auxilliary function calculates a new physical index
 */
int _dynarray_get_new_start_idx(struct dynarray* da) {
  assert(da);

  return (da->start + 1) < da->capacity ? da->start + 1 : 0;
}
...
void _dynarray_resize(struct dynarray* da, int new_capacity) {
  ...
  for (int i = 0; i < da->size; i++) {
    // For queue, start index floats so a physical index needs to be calculated
    int actualIdx = _dynarray_get_physical_idx(da, i);
    new_data[i] = da->data[actualIdx];
  }

  /*
   * Put the new array into the dynarray struct.
   * Reset start index to zero on the new array.
   */
  free(da->data);
  da->data = new_data;
...
void dynarray_insert(struct dynarray* da, void* val) {
   ...
   * For queue, start index floats so a new physical index needs to be calculated
   */
  int newIdx = _dynarray_get_physical_idx(da, da->size);
...
void dynarray_remove(struct dynarray* da, int idx) {
  ...
  // For queue, start index floats so a physical index needs to be calculated
  int actualIdx = _dynarray_get_physical_idx(da, idx);
  da->data[actualIdx] = NULL;

  // Index = 0 is equivalent to removing the first element, so we recalculate the start index
  if (idx == 0) da->start = _dynarray_get_new_start_idx(da);
...
void dynarray_set(struct dynarray* da, int idx, void* val) {
  ...
  // For queue, start index floats so a physical index needs to be calculated
  int actualIdx = _dynarray_get_physical_idx(da, idx);
  da->data[actualIdx] = val;
}

/* Additional functions to make queue possible */
/* int _dynarray_get_physical_idx(struct dynarray* da, int idx); */

/* int _dynarray_get_new_start_idx(struct dynarray* da); */

/*
 * This function returns and removes the first element off the array and calculates
 * a new start index.
 * 
 * Params:
 *    da - the array from which to return and remove the first element. May not be NULL.
 */
void* dynarray_return_front_and_remove(struct dynarray* da) {
  assert(da);

  void* returnValue = dynarray_get(da, 0);
