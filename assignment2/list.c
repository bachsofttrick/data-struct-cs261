...
/* Additional functions to make stack possible */
...
/*
 * This function should return the value stored at the top of a given list
 * *without* removing that value.
 *
 * Params:
 *   list - the list from which to query the top value. May not be NULL.
 */
void* list_view_head_value(struct list* list) {
  assert(list);

	return !list->head ? NULL : list->head->val;
}

/*
 * This function returns and removes the first element off the list and pushes the next one to
 * be the new head.
 * 
 * Params:
 *    list - the list from which to return and remove the first element. May not be NULL.
 */
void* list_return_head_and_remove(struct list* list) {
  assert(list);
  
  if (!list->head) return NULL;
  struct node* elmToFree = list->head;
  void* returnVal = elmToFree->val;
