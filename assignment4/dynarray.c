...
/*
 * This function exchanges the values of two existing elements in a dynamic array.
 *
 * Params:
 *   da - the dynamic array in which to exchanges values.  May not be NULL.
 *   first_idx - the index of the first element to exchange with.  The value
 *     of `first_idx` must be between 0 (inclusive) and n (exclusive), where n is the
 *     number of elements stored in the array minus 1, and must not be equal to the second index.
 *   second_idx - the index of the first element to exchange with.  The value
 *     of `second_idx` must be between 0 (inclusive) and n (exclusive), where n is the
 *     number of elements stored in the array minus 1, and must not be equal to the first index.
 */
void dynarray_exchange_elements(struct dynarray* da, int first_idx, int second_idx) {
  assert(da);
  assert(first_idx >= 0 && second_idx >= 0 && first_idx != second_idx && first_idx < da->size && second_idx < da->size);

  void* tempValue = da->data[first_idx];
  da->data[first_idx] = da->data[second_idx];
  da->data[second_idx] = tempValue;
}